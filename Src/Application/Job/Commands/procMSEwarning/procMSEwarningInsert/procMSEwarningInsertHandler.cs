...
public class procMSEwarningInsertHandler : IRequestHandler<procMSEwarningInsertCommand, IList<MSEwarning_ID>>
{
	readonly IMapper _mapper;
	readonly IProcedureAdabter _procedureAdabter;
	public procMSEwarningInsertHandler(IMapper mapper, IProcedureAdabter procedureAdabter)
	{
		_mapper = mapper;
		_procedureAdabter = procedureAdabter;
	}

	public async Task<IList<MSEwarning_ID>> Handle(procMSEwarningInsertCommand request, CancellationToken cancellationToken)
	{
		...
		IList<MSEwarning_ID> result = await _procedureAdabter
				.Execute<MSEwarning_ID>("[dbo].[proc_Enterprise_WarningInsert]",
					(nameof(ID), ID),
					(nameof(TIN), TIN),
